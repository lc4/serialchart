...
void Chart::newPacket(DecoderBase* decoder){
	...
	QPainter p(&px);
	int interval = config->get("_setup_","interval").toInt();
	int sections = MIN(packetValues.length(),config->fields.length());
	//clear one vertical line
	p.setCompositionMode(QPainter::CompositionMode_Clear);
	p.fillRect(cursor_pos,0,20,height(), Qt::red);
	p.setCompositionMode(QPainter::CompositionMode_SourceOver);
	...
	cursor_pos = (cursor_pos + interval) % width();
